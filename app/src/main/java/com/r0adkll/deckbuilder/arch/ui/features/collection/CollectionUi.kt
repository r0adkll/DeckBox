...
interface CollectionUi : Ui<CollectionUi.State, CollectionUi.State.Change> {
    ...
    data class State(
        ...
        override fun reduce(change: Change): Ui.State<Change> = when (change) {
            ...
            is Change.MigrationNeeded -> copy(
                isMigrationNeeded = change.isMigrationNeeded,
                isMigrationInProgress = if (!change.isMigrationNeeded) false else isMigrationInProgress
            )
