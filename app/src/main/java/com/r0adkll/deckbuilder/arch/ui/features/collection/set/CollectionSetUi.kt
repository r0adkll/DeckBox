...
interface CollectionSetUi : Ui<State, Change> {
    ...
    data class State(
        ...
        sealed class Change(logText: String) : Ui.State.Change(logText) {
            ...
            class CountsUpdated(
                val counts: List<CollectionCount>
            ) : Change("network -> counts updated (${counts.size})")
            class CountChanged(
                val card: PokemonCard,
                val count: Int
            ) : Change("user -> count changed ($count)")
            class CountsChanged(
                val cards: List<PokemonCard>,
                val count: Int
            ) : Change("user -> counts changed(${cards.size}, count=$count)")
            object ToggleMissingCards : Change("user -> toggled missing card visibility")
        }

        override fun toString(): String {
            return "State(isLoading=$isLoading, error=$error, expansion=${expansion?.code}, " +
                "cards=${cards.size}, counts=${counts.size})"
        }
