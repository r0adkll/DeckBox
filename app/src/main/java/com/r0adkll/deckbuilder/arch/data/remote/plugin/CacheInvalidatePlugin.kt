...
class CacheInvalidatePlugin(
    ...
    override fun onFetchActivated(remote: Remote) {
        ...
        remote.expansionVersion?.let { (versionCode, expansionCode) ->
            Timber.d("""Checking Expansion Cache (version: $versionCode, expansion: $expansionCode, 
                prefVersion: ${preferences.expansionsVersion})""".trimMargin())
            ...
            try {
                ...
                if (versionCode > preferences.expansionsVersion || invalidCache) {
                    Timber.i("""Expansion Cache Invalidated, Refreshing from server 
                        |(version: $versionCode, expansion: $expansionCode)""".trimMargin())
                    expansionDataSource.refreshExpansions()
                        .subscribeOn(schedulers.network)
                        .subscribe({
                            Timber.d("""Expansions refreshed, updating version
                                |(${preferences.expansionsVersion} > $versionCode)""".trimMargin())
