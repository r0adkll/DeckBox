...
sealed class Event {
    ...
    sealed class SelectContent(
        ...
        sealed class Deck(override val id: String, override val name: String? = null) : SelectContent("deck", id) {
            ...
            data class EditCollectionOnly(
                val enabled: Boolean
            ) : Deck("change_collection_only", if (enabled) "on" else "off")
        }
        ...
        ) : SelectContent("action", id, name, value)

        data class MenuAction(
            override val id: String,
            override val value: Long? = null
        ) : SelectContent("menu_action", id, value = value)
